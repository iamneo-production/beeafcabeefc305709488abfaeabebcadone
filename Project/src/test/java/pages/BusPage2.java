...
public class BusPage2 extends Base {
    ...
    public void Source() {
        try {
            String source1 = file.ReadData("./testdata/Testdata.xlsx", "Abhibus", 1, "Source");
...
//            test.log(Status.PASS,"Source Value passed");
         ...
         // Send Enter key to select the highlighted item
        	...
        	logger.error("Exception occurred while Entering Source: " + e.getMessage(), e);
            String base64Screenshot = Reporter.captureScreenshotAsBase64(driver, "Bus Source i/p Error");
            test.fail("Failed to Input source  ", MediaEntityBuilder.createScreenCaptureFromBase64String(base64Screenshot).build());
        }
    }
    ...
    {
    try {
    	 String source2 = file.ReadData("./testdata/Testdata.xlsx", "Abhibus", 1, "Destination");
...
//         test.log(Status.PASS,"Source Value passed");
         ...
         test.log(Status.PASS, "Selected the desired suggestion: " + source2);
    		...
    		logger.error("Exception occurred while performing I/p dest search: " + e.getMessage(), e);
            String base64Screenshot = Reporter.captureScreenshotAsBase64(driver, "Bus dest I/P Error");
            test.fail("Failed to perform dest I/P", MediaEntityBuilder.createScreenCaptureFromBase64String(base64Screenshot).build());
    	}
    ...
    try {
    	...
    	WebDriverHelper.ClickOnElement(BusUI.datepick);
         ...
         test.log(Status.PASS,"Clicked Date");
    		...
    		logger.error("Exception occurred while performing date selection: " + e.getMessage(), e);
            String base64Screenshot = Reporter.captureScreenshotAsBase64(driver, "Date I/P Error");
            test.fail("Failed to perform Date picker action", MediaEntityBuilder.createScreenCaptureFromBase64String(base64Screenshot).build());
    	}
...
try {
	...
	WebDriverHelper.ClickOnElement(BusUI.click_search);
     ...
     String Kyword1 = "Bangalore";
     String Kyword2 = "Coimbatore";
     ...
     if(ast.contains(Kyword1)&& ast.contains(Kyword2) )
     {
		...
		logger.error("Exception occurred while performing final search: " + e.getMessage(), e);
        String base64Screenshot = Reporter.captureScreenshotAsBase64(driver, "Search click Error");
        test.fail("Failed to perform search", MediaEntityBuilder.createScreenCaptureFromBase64String(base64Screenshot).build());
	}
